...
const VEHICLE_MAP = {
  ...
  KIA: ['Kia', 'Seltos', 2023], TAT: ['Tata', 'Nexon', 2023],
...
async function route(req, res) {
  ...
  if (p === '/api/auth/register' && method === 'POST') {
    ...
    const newUser = {
      id: DB._uid++, name: b.name, email: b.email, password_hash: hashPw(b.password),
  ...
  if (p === '/api/auth/login' && method === 'POST') {
    ...
    setSession(res, u.id);
    return json(res, {
      success: true, is_admin: u.is_admin, user: {
  ...
  if (p === '/api/ecu/live' && method === 'GET') {
    ...
    const log = {
      id: DB._tid++, user_id: user.id, session_id: sessionId,
  ...
  if (p === '/api/ecu/ingest' && method === 'POST') {
    ...
    const pred = predict(ecu);
    const log = {
      id: DB._tid++, user_id: user.id, session_id: sessionId, timestamp: new Date().toISOString(),
  ...
  if (p === '/api/metrics' && method === 'GET') {
    ...
    return json(res, {
      ...
      stats: {
        total_logs: DB.telemetry.filter(t => t.user_id === user.id).length,
