...
function startTestServer(){
  ...
  DB.users.push({ id:DB._u++, name:'Administrator', email:'admin@ecu.com',
  ...
  const VMAP={TYT:['Toyota','Innova Crysta',2022],HON:['Honda','City',2023],MAR:['Maruti','Swift',2021],
    HYN:['Hyundai','Creta',2023],KIA:['Kia','Seltos',2023],TAT:['Tata','Nexon EV',2023],
  ...
  function simRow(){
    ...
    return{engine_rpm:+ss.rpm.toFixed(1),vehicle_speed:+ss.speed.toFixed(1),throttle_position:+ss.thr.toFixed(1),
  ...
  function mlPredict(f){
    const F={engine_rpm:+f.engine_rpm||0,vehicle_speed:+f.vehicle_speed||0,throttle_position:+f.throttle_position||0,
    ...
    return{fuel_xgb:+xgb.toFixed(3),fuel_ridge:+ridge.toFixed(3),fuel_svr:+svr.toFixed(3),
  ...
  function setSession(res,uid){const sid=crypto.randomBytes(16).toString('hex');DB.sess[sid]=uid;
  ...
  const server=createServer(async(req,res)=>{
    ...
    if(p==='/api/auth/register'&&m==='POST'){
      ...
      const nu={id:DB._u++,name:b.name,email:b.email,password_hash:hashPw(b.password),vehicle_api_key:b.vehicle_api_key,
    ...
    if(p==='/api/ecu/live'&&m==='GET'){
      ...
      const log={id:DB._t++,user_id:user.id,session_id:sid,timestamp:new Date().toISOString(),...ecu,
